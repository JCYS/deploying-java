...
import com.copeinca.apicopeincaprov.modules.core.models.entities.SvProveedorSedeEntity_;
...
/**
 * Specification para búsqueda dinámica de SvProveedorSedeEntity.
 * Extiende de AbstractSpecification para reutilizar lógica común de filtros.
 */
public class SvProveedorSedeSpecification extends AbstractSpecification {
     ...
     */
    public static Specification<SvProveedorSedeEntity> byFilter(SvProveedorSedeFilter filter) {

        return (Root<SvProveedorSedeEntity> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {

            List<Predicate> predicates = new ArrayList<>();

            //--- Filtro dinámico para id
            addPredicates(predicates, cb, root, SvProveedorSedeEntity_.id, filter.getId());

            //--- Filtro dinámico para isActive
            addPredicates(predicates, cb, root, SvProveedorSedeEntity_.isActive, filter.getIsActive());

            //--- Filtro FK por IDs: proveedorId
            if (filter.getProveedorIdIds() != null && !filter.getProveedorIdIds().isEmpty())
                predicates.add(root.get(SvProveedorSedeEntity_.proveedorId).in(filter.getProveedorIdIds()));

            //--- Filtro FK por IDs: sedeId
            if (filter.getSedeIdIds() != null && !filter.getSedeIdIds().isEmpty())
                predicates.add(root.get(SvProveedorSedeEntity_.sedeId).in(filter.getSedeIdIds()));
