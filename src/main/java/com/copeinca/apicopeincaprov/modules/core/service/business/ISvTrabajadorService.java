/*
 * Autor: Anthony Ramos
 * Email: anthonyramosdev@gmail.com
 * Template pack-angular:src/main/java/modules/core/service/business/IEntityService.java.e.vm
 */
package com.copeinca.apicopeincaprov.modules.core.service.business;

import com.copeinca.apicopeincaprov.modules.core.models.dto.SvTrabajadorDTO;
import com.copeinca.apicopeincaprov.modules.core.models.dto.SvTrabajadorRegistroDTO;
import org.springframework.web.multipart.MultipartFile;

import java.util.List;

/**
* Interface de servicio compuesta para SvTrabajadorEntity
* Combina todas las funcionalidades especializadas: consulta, lectura, escritura y CRUD.
* Los desarrolladores pueden implementar solo las interfaces específicas que necesiten
* o usar esta interfaz completa para funcionalidad completa.
*/
public interface ISvTrabajadorService {

    // Esta interfaz hereda todos los métodos de las interfaces especializadas
    // Permite flexibilidad máxima en la implementación según las necesidades del negocio

    SvTrabajadorRegistroDTO registrarTrabajador(SvTrabajadorRegistroDTO trabajador, List<MultipartFile> aFilesSSO, List<MultipartFile> aFilesAAR,List<MultipartFile> aFilesP) throws Exception;

    SvTrabajadorRegistroDTO getTrabajador(String id);

    List<SvTrabajadorDTO> isExist(List<SvTrabajadorDTO> result);

    boolean findFiles(String id);
}
