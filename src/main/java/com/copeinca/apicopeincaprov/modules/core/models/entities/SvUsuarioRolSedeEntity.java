...
@Entity
@Table(name = "SV_USUARIO_ROL_SEDE")
public class SvUsuarioRolSedeEntity extends BaseEntity implements Serializable {

    // -----------------------------------------------------------------
    // Attributes
    // -----------------------------------------------------------------

    @Column(name = "USUARIO_ROL_SEDE_ID", length = 36)
    ...
    @Column(name = "ROL_CODE", nullable = false, length = 50)
    ...
    @JoinColumn(name = "ROL_CODE", nullable = false, insertable = false, updatable = false)
    @ManyToOne
    private SvRolEntity rolCodeEntity;

    @JoinColumn(name = "USUARIO_ID", nullable = false, insertable = false, updatable = false)
    @ManyToOne
    private SvUsuarioEntity usuarioIDEntity;
